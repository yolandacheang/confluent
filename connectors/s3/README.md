...
```
$ docker-compose exec schema-registry kafka-avro-console-consumer -bootstrap-server kafka:9092 --topic dbserver1.mydb.team --from-beginning --property schema.registry.url=http://localhost:8082
...
```
./scripts/s3-connector
```

You should see topics and partitions created in minio web interface on http://localhost:9000.
...
You can also list the S3 bucket 
